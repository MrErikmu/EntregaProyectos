...
public class VehiculoValidator: IValidator<Vehiculo>
{
    public Result<Vehiculo, DomainError> Validar(Vehiculo vh)
    {
        ...
        if(vh.FechaInspeccion<DateTime.Today||vh.FechaInspeccion>DateTime.Today.AddDays(30))
           errores.Add($"La Fecha de Inspeccion debe estar comprendida entre {DateTime.Today.ToShortDateString()} y 30 dias despues");
        if(!IsokDNI(vh.DNI))
        ...
        if (errores.Any())
        {
            return Result.Failure<Vehiculo, DomainError>(new VehiculoError.ValidationError(errores));
        }
        return Result.Success<Vehiculo, DomainError>(vh);
    }
